...
WEIGHT_URL = 'https://github.com/fchollet/deep-learning-models/releases/download/v0.7/inception_resnet_v2_weights_tf_dim_ordering_tf_kernels.h5'
...
def InceptionResNetV2(img_rows, img_cols, color_type=1, num_classes=None):
    ...
    xx = GlobalAveragePooling2D(name='avg_pool')(x)
    xx = Dense(1000, activation='softmax', name='predictions')(xx)

    # Create model
    model = Model(img_input, xx, name='inception_resnet_v2')

    # Load weights
    model.load_weights("../weight/inception_resnet_v2_weights_tf.h5")
